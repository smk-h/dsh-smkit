/**
 * English dictionary for the `mcp` locale namespace.
 *
 * Key set must stay identical to `zh.ts`. This is also the fallback DSH
 * uses when the resolved language has no `mcp` dictionary.
 */

import type { LocaleDict } from '../../../platform/types'

export const MCP_LOCALE_EN: LocaleDict = {
  "sectionLabel": "MCP",
  "breadcrumb": "Breadcrumb",
  "countGlobal": "Global {count}",
  "countWorkspace": "Workspace {count}",
  "namePlaceholder": "odin",
  "commandPlaceholder": "npx",
  "cwdPlaceholder": "/path/to/project",
  "urlPlaceholder": "https://mcp.example.com/mcp",
  "tokenEnvPlaceholder": "MCP_BEARER_TOKEN",
  "enableFailed": "Enable failed (HTTP {status})",
  "disableFailed": "Disable failed (HTTP {status})",
  "addFailed": "Add failed (HTTP {status})",
  "connected": "Enabled",
  "needs-auth": "Authentication required",
  "authorizing": "Authenticating",
  "connecting": "Connecting",
  "reconnecting": "Reconnecting",
  "error": "Error",
  "disconnected": "Disconnected",
  "disabled": "Disabled",
  "conflict": "Conflict",
  "configured": "Configured",
  "staticToken": "Static token",
  "disable": "Disable",
  "enable": "Enable",
  "reauth": "Reauthenticate",
  "auth": "Authenticate",
  "edit": "Edit",
  "deleteServer": "Delete MCP server",
  "chooseWorkspaceError": "Please select a workspace",
  "scope": "Scope",
  "userScope": "user (global, available in all workspaces)",
  "workspaceScope": "workspace (available only in the selected workspace)",
  "workspace": "Workspace",
  "chooseWorkspace": "Select a workspace…",
  "type": "Type",
  "http": "HTTP (remote server)",
  "stdio": "stdio (local process)",
  "name": "Name",
  "command": "Command (executable)",
  "arguments": "Arguments",
  "argumentValue": "Argument value",
  "addArgument": "＋ Add argument",
  "environment": "Environment variables",
  "keyUpper": "KEY",
  "valueUpper": "VALUE",
  "addEnvironment": "＋ Add environment variable",
  "cwd": "Working directory (optional)",
  "url": "MCP server URL",
  "authMode": "Authentication method",
  "oauth": "OAuth (authenticate in browser)",
  "bearer": "Static Bearer token",
  "noAuth": "No auth",
  "noAuthOption": "No auth (server needs no credentials)",
  "tokenEnv": "Bearer token environment variable (name, not the token itself)",
  "headers": "Headers",
  "key": "Key",
  "value": "Value",
  "addHeader": "＋ Add header",
  "headerEnv": "Headers from environment variables (enter variable names as values)",
  "envName": "Environment variable name",
  "addVariable": "＋ Add variable",
  "save": "Save",
  "cancel": "Cancel",
  "advanced": "Advanced",
  "advancedTitle": "Advanced settings",
  "toolCallTimeout": "Tool call timeout (ms)",
  "toolCallTimeoutHelp": "Applies to tools/call only; initialize and tools/list keep their fixed 60 seconds. The default is 60000 (60 s); a saved value is written to ~/.dsh/mcp-manager.json and applies immediately, without reconnecting the server.",
  "restoreDefault": "Restore default",
  "invalidTimeout": "The timeout must be a whole number of milliseconds of at least 1.",
  "saveTimeoutFailed": "Save failed (HTTP {status})",
  "autoReconnect": "Automatic reconnect",
  "reconnectMaxAttempts": "Max reconnect attempts",
  "reconnectMaxDelayMs": "Max retry delay (ms)",
  "healthCheckIntervalMs": "Health check interval (ms)",
  "reconnectHelp": "Rebuilds a connection that dropped: a stdio child exiting, or a failed health check (a periodic tools/list), both trigger it. Retries start at 1 s and double up to the max delay; 0 attempts means retry forever; an interval of 0 turns the health check off and leaves only the exit signals.",
  "invalidReconnectNumber": "All three fields must be whole numbers of at least 0.",
  "saveReconnectFailed": "Save failed (HTTP {status})",
  "deleteWorkspaceServer": "Delete workspace MCP server",
  "hide": "Hide",
  "addServer": "Add MCP server",
  "editGlobal": "Edit MCP server (global)",
  "editWorkspace": "Edit MCP server (workspace)",
  "servers": "MCP servers",
  "onDemand": "On-demand MCP tool calls",
  "onDemandHelp": "In Native mode, MCP exposes only the search, describe, and execute broker tools",
  "on": "On",
  "off": "Off",
  "global": "Global",
  "workspaceServers": "Workspace servers (visible only in this workspace)",
  "emptyWorkspace": "No MCP servers in this workspace yet. Click ＋ above to add one (saved to .dsh/dshmm/mcp.json).",
  "globalServers": "Global servers (available in this workspace)",
  "emptyGlobal": "No global servers yet. Switch to the Global view to add one.",
  "search": "Search MCP servers…",
  "clearSearch": "Clear search",
  "searchEmpty": "No matching MCP servers",
  "openConfig": "Open config file",
  "openConfigFailed": "Failed to open config file (HTTP {status})",
  "authFailed": "Failed to start authentication (HTTP {status})",
  "deleteFailed": "Delete failed (HTTP {status})",
  "toggleFailed": "Toggle failed (HTTP {status})",
  "saveFailed": "Save failed (HTTP {status})",
  "toolCount": "Tools: {count}",
  "toolKind": "[tool]",
  "noDescription": "(this tool has no description)",
  "noParameters": "No parameters",
  "required": "required",
  "confirmDelete": "Delete “{name}”? This cannot be undone.",
  "confirmWorkspaceDelete": "Delete “{name}” from this workspace?",
  "restart": "Restart",
  "stop": "Stop",
  "restartFailed": "Restart failed (HTTP {status})",
  "stopFailed": "Stop failed (HTTP {status})",
}
