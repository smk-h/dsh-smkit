...
export const OPENSPEC_LOCALE_ZH: LocaleDict = {
  ...
  "openSpecUntrack": "移除忽略并重新跟踪",
  "openSpecUntrackCommand": "「加入 .gitignore」的反向操作：\nopenspec/.gitignore 由该按钮创建，整个删除\n共享目录（如 .agents/skills）的 .gitignore 只去掉指向 OpenSpec 条目的行，文件若只写着这些内容则连文件一起删，其余内容原样保留\n然后逐项问 git：已在跟踪的只报告、不动；仍被别人规则（顶层 .gitignore、info/exclude、全局配置）挡住的只报告、不强加；其余用 git add 重新纳入跟踪——只入暂存区，不自动提交",
  "openSpecUntrackRunning": "正在移除忽略并重新跟踪…",
  "openSpecUntrackFailed": "移除忽略失败 (HTTP {status})",
  "openSpecUntrackUnlisted": "已从忽略文件去掉规则：{count} 项",
  "openSpecUntrackRetracked": "已重新纳入跟踪：{count} 项（已暂存，未提交）",
  "openSpecUntrackTracked": "本就在跟踪中：{count} 项（未改动）",
  "openSpecUntrackAlreadyUnlisted": "忽略文件里本就没有它的行：{count} 项",
  "openSpecUntrackStillIgnored": "仍被其他忽略规则挡住：{count} 项（不改动不属于自己的规则，也未强行添加）",
