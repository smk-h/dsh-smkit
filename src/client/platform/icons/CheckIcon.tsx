 ...
 * (`IconCheckOutline16`), so a picker draws the same shapes DSH does without
 * taking a runtime dependency on that package.
 *
 * It sits beside `Icon.tsx` rather than in a feature because the shared picker
 * (`platform/ui/IconSelect`) draws it, and more than one settings page mounts
 * that picker (the icon convention above gives a glyph with more than one owner
 * one home here).
 */
