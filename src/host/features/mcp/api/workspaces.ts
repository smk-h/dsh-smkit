...
async function respondWithWorkspaces(api: McpApiDeps, res: Parameters<McpHandler>[1]): Promise<void> {
  sendJson(res, 200, { workspaces: api.workspaces.listWorkspaces() })
}

export const handleWorkspaces: McpHandler = async (req, res, facts, api) => {
  const { rest } = facts
